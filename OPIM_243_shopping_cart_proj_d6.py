...
#this has to change with the identifier moving from int to string!
def create_cost_dictionary(product_list, product_identifier_dict, tax_rate):
    ...
    for key in product_identifier_dict:
        ...
        subtotal = subtotal + (float(product_list[index]['price']) * float(product_identifier_dict[identifier]))
...
#test function 
#cost_dictionary = create_cost_dictionary(product_identifier_dict, 0.0725)
...
def print_item_and_price(product_list, product_identifier_dict, identifier):
    ...
    for i in range(0, len(product_list)):
        if product_list[i]['id'] == identifier:
...
'''
def print_item_and_price(product_list, product_identifier_dict, identifier):
    if product_list[identifier-1]['price_per'] == 'item':
        return('+ ' + str(product_identifier_dict[identifier]) + ' ' +
              product_list[identifier-1]['name'] + ' ($' +
              str('%0.2f' % round(float(product_list[identifier-1]['price'] *
                          float(product_identifier_dict[identifier])),2) ) + ')')
    elif product_list[identifier-1]['price_per'] == 'pound':
        return('+ ' + str(product_identifier_dict[identifier]) + ' lbs ' +
              product_list[identifier-1]['name'] + ' at ' +
              str(product_list[identifier-1]['price']) + '/lb ($' +
              str('%0.2f' % round(float(product_list[identifier-1]['price'] *
                          float(product_identifier_dict[identifier])),2) ) + ')')   '''     
    

#test print item and price function
# print_item_and_price(products, product_identifier_dict, 21)


def print_receipt(product_list, product_identifier_dict, cost_dictionary):

    with open(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S-%f')+'.txt', 'w') as text_file:
        print('---------------------------------', file=text_file)
        print('Madeline Lee\'s Grocery Store (: ', file=text_file)
        print('---------------------------------', file=text_file)
        print('Web: github.com/madelinenlee/OPIM_243_shopping_cart_project', file=text_file)
        print('Phone: 123-456-5789', file=text_file)
        print('Checkout time: ', datetime.datetime.now(), file=text_file)
        print('---------------------------------', file=text_file)
        print('Shopping Cart Items: ', file=text_file)
        for identifier in product_identifier_dict:
            temp_string = print_item_and_price(product_list, product_identifier_dict, identifier)
            print(temp_string, file=text_file)
        print('---------------------------------', file=text_file)
        print('Subtotal: $' + str('%0.2f' % cost_dictionary['subtotal']), file=text_file)
        print('Plus CA Sales Tax: $' + str('%0.2f' % cost_dictionary['added_tax']), file=text_file)
        print('Total: $' + str('%0.2f' % cost_dictionary['total_cost']), file=text_file)
        print('---------------------------------', file=text_file)
        print('Thank you for your business! Please come again.', file=text_file)
    text_file.close()
    
    
#test print_receipt function
# print_receipt(products, product_identifier_dict, cost_dictionary)

def run_shopping_cart(product_list, tax_rate):
    product_identifier_dict = user_inputs(product_list)
    cost_dictionary = create_cost_dictionary(product_identifier_dict, tax_rate)
    print_receipt(product_list, product_identifier_dict, cost_dictionary)
